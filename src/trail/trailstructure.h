...
class TrailStructure {
 private:
    ...
    std::unique_ptr<std::vector<uint>> married;
 ...
 public:
     ...
     */
    explicit TrailStructure(uint _degree);
     ...
     */
    bool isEndingArc(uint i) const;

    /**
     * @return value of dyckStart
     */
    uint getDyckStart() const;
