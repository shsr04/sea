...
NaiveEulerTrail::NaiveEulerTrail(const shared_ptr<UndirectedGraph> &g) {
    static const std::tuple<uint, uint> nonArc =
        std::make_tuple((uint) -1, (uint) -1);

    uint order = g->getOrder();
    std::vector<Sealib::NaiveTrailStructure *> ts;
    for (uint i = 0; i < order; i++) {
        uint degree = g->getNode(i).getDegree();
    ...
    while (u != (uint) -1) {
        std::tuple<uint, uint> aOld = nonArc;
        ...
        if (u == (uint) -1) {  // no odd found, search for grey
            for (uint i = 0; i < order; i++) {
                // first that has edges, it's possible to have a graph with no edges
                if (ts[i]->isGrey()
                    && !ts[i]->isBlack()) {
        ...
        if (u == (uint) -1) {  // no odd found and no grey found, search for white
            for (uint i = 0; i < order; i++) {
                // first that has edges, it's possible to have a graph with no edges
                if (!ts[i]->isBlack()) {
                    u = i;
