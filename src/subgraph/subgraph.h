...
class SubGraph {
 ...
 protected:
    ...
    inline uint64_t select_q(uint64_t i) const { return qSelect->select(i); }

    inline uint64_t select_p(uint64_t i) const { return pSelect->select(i); }

    inline uint64_t rank_q(uint64_t i) const { return qSelect->rank(i); }

    inline uint64_t rank_p(uint64_t i) const { return pSelect->rank(i); }

 public:
    inline uint64_t getSidx() const { return sidx; }
    ...
    Result<uint64_t> degree(uint64_t u) const {
        ...
        uint64_t b =
            select_p(rank_q(u - 1));  // pSelect.select(qSelect.rank(v - 1));
    ...
    virtual std::tuple<uint64_t, uint64_t> mate(uint64_t u,
                                                uint64_t k) const = 0;

    inline std::tuple<uint64_t, uint64_t> mate(
        std::tuple<uint64_t, uint64_t> uk) const {
    ...
    inline uint64_t gMax() { return pSelect->size(); }
