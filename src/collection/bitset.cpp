...
Bitset<BlockType, AllocatorType>::Bitset(uint bits_) :
    bits(bits_),
...
Bitset<BlockType, AllocatorType>::Bitset(const std::vector<bool> &bitvector) :
    bits(static_cast<uint>(bitvector.size())),
    mbits(bits % bitsPerBlock == 0 ? bits / bitsPerBlock : bits / bitsPerBlock + 1) {
    uint index = 0;
...
Bitset<BlockType, AllocatorType>::operator[](uint bit) {
    assert(bit < bits);
    return BitReference(&mbits[bit / bitsPerBlock], bit % bitsPerBlock);
}

template<typename BlockType, typename AllocatorType>
bool Bitset<BlockType, AllocatorType>::operator[](uint bit) const { return get(bit); }
...
template<typename BlockType, typename AllocatorType>
bool Bitset<BlockType, AllocatorType>::get(uint bit) const {
    assert(bit < bits);
    return get(mbits[bit / bitsPerBlock], bit % bitsPerBlock);
}

template<typename BlockType, typename AllocatorType>
void Bitset<BlockType, AllocatorType>::insert(uint index, bool value) {
    operator[](index) = value;
}

template<typename BlockType, typename AllocatorType>
const BlockType &Bitset<BlockType, AllocatorType>::getBlock(uint idx) const {
    assert(mbits.size() > idx);
    return mbits[idx];
}

template<typename BlockType, typename AllocatorType>
void Bitset<BlockType, AllocatorType>::setBlock(uint idx, BlockType block) {
    assert(mbits.size() > idx);
    mbits[idx] = block;
}
...
template<typename BlockType, typename AllocatorType>
BlockType Sealib::Bitset<BlockType, AllocatorType>::getShiftedBlock(uint idx) const {
