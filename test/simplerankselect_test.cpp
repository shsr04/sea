...
TEST(SimpleRankSelectTest, simpleRankSelect) {
    ...
    ASSERT_EQ((uint) - 1, simpleRankSelect.select(0));
    ASSERT_EQ((uint) - 1, simpleRankSelect.rank(0));
    ASSERT_EQ((uint) - 1, simpleRankSelect.select(2));
    ASSERT_EQ((uint) - 1, simpleRankSelect.rank(2));

    (*bits)[0] = 0;
    simpleRankSelect = Sealib::SimpleRankSelect(bits);
    ASSERT_EQ((uint) - 1, simpleRankSelect.select(1));
